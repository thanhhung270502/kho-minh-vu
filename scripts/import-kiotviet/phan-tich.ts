...
import { docSheet, doChuoi, doSo } from "./doc-file";
...
async function main() {
  ...
  const dvt = dem(sp.map((d) => doChuoi(d.o["dvt"])));
  console.log(`\nĐVT (${dvt.length} giá trị):`);
  for (const [k, n] of dvt) console.log(`  ${String(n).padStart(5)}  ${k}`);

  const nhom = dem(sp.map((d) => doChuoi(d.o["nhom_hang_3_cap"])));
  ...
  const quyDoi = dem(sp.map((d) => doChuoi(d.o["quy_doi"])));
  console.log(`\nQuy đổi: ${quyDoi.map(([k, n]) => `${k}=${n}`).join(" · ")}`);

  const dvtCoBan = dem(sp.map((d) => doChuoi(d.o["ma_dvt_co_ban"])));
  console.log(`Mã ĐVT Cơ bản (${dvtCoBan.length} giá trị, 8 đầu): ${dvtCoBan.slice(0, 8).map(([k, n]) => `${k}=${n}`).join(" · ")}`);

  const loai = dem(sp.map((d) => doChuoi(d.o["loai_hang"])));
  console.log(`Loại hàng: ${loai.map(([k, n]) => `${k}=${n}`).join(" · ")}`);

  const kd = dem(sp.map((d) => doChuoi(d.o["dang_kinh_doanh"])));
  console.log(`Đang kinh doanh: ${kd.map(([k, n]) => `${k}=${n}`).join(" · ")}`);

  const ma = sp.map((d) => doChuoi(d.o["ma_hang"]));
  ...
  const giaBan = sp.map((d) => doSo(d.o["gia_ban"]) ?? 0);
  console.log(`Giá bán > 0: ${giaBan.filter((g) => g > 0).length}/${sp.length}`);

  const ton = sp.map((d) => doSo(d.o["ton_kho"]) ?? 0);
  ...
  const viTri = sp.filter((d) => doChuoi(d.o["vi_tri"])).length;
  const hinh = sp.filter((d) => doChuoi(d.o["hinh_anh_url1_url2"])).length;
  const moTa = sp.filter((d) => doChuoi(d.o["mo_ta"])).length;
  const tonMin = sp.filter((d) => (doSo(d.o["ton_nho_nhat"]) ?? 0) > 0).length;
  console.log(`Có vị trí: ${viTri} · có hình: ${hinh} · có mô tả: ${moTa} · tồn nhỏ nhất > 0: ${tonMin}`);

  // ─── Nhà cung cấp ───
  const ncc = await docSheet(tim("DanhSachNhaCungCap"));
  console.log(`\n═══ NHÀ CUNG CẤP — ${ncc.length} dòng`);
  for (const d of ncc) {
    const m = doChuoi(d.o["ma_nha_cung_cap"]) ?? "";
    console.log(
      `  ${m.padEnd(12)} ${String(doChuoi(d.o["ten_nha_cung_cap"]) ?? "").slice(0, 40).padEnd(40)}` +
        ` MST=${doChuoi(d.o["ma_so_thue"]) ?? "—"}  nhóm=${doChuoi(d.o["nhom_nha_cung_cap"]) ?? "—"}` +
        `  TT=${doChuoi(d.o["trang_thai"]) ?? "—"}  tổng mua=${doChuoi(d.o["tong_mua"]) ?? "—"}`,
    );
  }
  ...
  const nccNhap = dem(nhap.map((d) => `${doChuoi(d.o["ma_nha_cung_cap"])} ${doChuoi(d.o["ten_nha_cung_cap"])}`));
  for (const [k, n] of nccNhap) console.log(`  ${String(n).padStart(4)}  ${k}`);
  const maSp = new Set(ma.filter(Boolean));
  const maNhapLa = dem(nhap.map((d) => doChuoi(d.o["ma_hang"])).filter((m) => m && !maSp.has(m)));
  ...
  const kh = dem(hd.map((d) => `${doChuoi(d.o["ma_khach_hang"])} ${doChuoi(d.o["ten_khach_hang"])}`));
  console.log(`  Khách hàng (${kh.length}): ${kh.slice(0, 5).map(([k, n]) => `${k}×${n}`).join(" · ")}`);
  const ghiChu = dem(hd.map((d) => doChuoi(d.o["ghi_chu"])));
  ...
  const donGia = hd.filter((d) => (doSo(d.o["don_gia"]) ?? 0) > 0).length;
  console.log(`  Đơn giá > 0: ${donGia}/${hd.length}`);
  const maHdLa = dem(hd.map((d) => doChuoi(d.o["ma_hang"])).filter((m) => m && !maSp.has(m)));
  console.log(`  Mã hàng trên hóa đơn KHÔNG có trong danh mục: ${maHdLa.length}${maHdLa.length ? " → " + maHdLa.map(([k, n]) => `${k}×${n}`).join(", ") : ""}`);
  const trangThai = dem(hd.map((d) => doChuoi(d.o["trang_thai"])));
