...
function readDate(value: string | null): string {
  if (!value) return "";
  if (!DATE_FORMAT.safeParse(value).success) return "";

  // Regex chỉ kiểm đúng khuôn số — "2026-13-45" vẫn khớp. Phải dựng lại ngày
  // rồi so ngược ba phần, không thì tháng/ngày vô lý lọt qua bộ lọc.
